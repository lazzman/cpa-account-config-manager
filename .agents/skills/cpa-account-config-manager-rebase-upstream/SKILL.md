---
name: cpa-account-config-manager-rebase-upstream
description: 安全地将当前 cpa-account-config-manager 分支的本地提交 rebase 到作者仓库 Mxucc/cpa-account-config-manager 的 upstream/main，保护并恢复未提交改动，验证重放结果，并在用户明确要求发布时仅以 --force-with-lease 推送到个人 origin。用于“同步上游”“rebase upstream”“同步 fork”“将本地改动接到最新版 main”“强推到 origin”或“拉取 Mxucc/cpa-account-config-manager 更新”。
---

# CPA Account Config Manager 上游安全 Rebase

将个人仓库的本地提交安全重放到作者上游。默认仅同步和验证；仅在用户明确要求发布时才安全强推。

| 角色 | 远程 | 仓库 | 用途 |
| --- | --- | --- | --- |
| 作者仓库 | `upstream` | `https://github.com/Mxucc/cpa-account-config-manager` | 拉取与 rebase 基线 |
| 个人仓库 | `origin` | `https://github.com/lazzman/cpa-account-config-manager.git` | 发布目标 |
...
- 粘性代理 URL 模板（`internal/manager/proxy_template.go`、批量编辑与相关测试）
- 本仓库 CPA 插件商店 `registry.json` 与安装说明
- 默认更新仓库指向 `lazzman/cpa-account-config-manager`
- 上游安全同步技能与定时任务脚本
...
```bash
bash .agents/skills/cpa-account-config-manager-rebase-upstream/scripts/rebase_upstream.sh
# 或显式指定作者分支：
bash .agents/skills/cpa-account-config-manager-rebase-upstream/scripts/rebase_upstream.sh upstream/main
```
