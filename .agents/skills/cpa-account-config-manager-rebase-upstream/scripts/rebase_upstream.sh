#!/usr/bin/env bash
# cpa-account-config-manager 专属：校验双远程，抓取作者上游，并安全地重放当前分支提交。
set -euo pipefail

readonly EXPECTED_ORIGIN_URL="https://github.com/lazzman/cpa-account-config-manager.git"
readonly EXPECTED_UPSTREAM_URL="https://github.com/Mxucc/cpa-account-config-manager"
...
默认将当前 cpa-account-config-manager 分支安全 rebase 到 upstream/main。
脚本会校验 origin=lazzman/cpa-account-config-manager、upstream=Mxucc/cpa-account-config-manager，
保护未提交与未跟踪改动，在抓取远程后启动 rebase；脚本绝不会推送。
...
# 通过核心 Go 模块、前端工程和原生桥接文件阻止在其他仓库误执行。
if [[ ! -f go.mod || ! -f web/package.json || ! -f cgo_bridge.go || ! -f Makefile ]] \
  || ! grep -qx 'module cpa-account-config-manager' go.mod; then
  fail "当前仓库不像 cpa-account-config-manager（缺少核心项目文件或模块标识）"
fi
...
if [[ -n "$(git status --porcelain)" ]]; then
  stash_label="cpa-account-config-manager-rebase-upstream-${branch}-$(date +%Y%m%d%H%M%S)"
