...
const markers = [
  ...
  join(cwd, 'cgo_bridge.go'),
  join(cwd, 'Makefile'),
];
if (markers.some((p) => !existsSync(p))) {
  console.error('[pre-run] not cpa-account-config-manager workingDir:', cwd);
  process.exit(1);
}
...
if (!/^module cpa-account-config-manager$/m.test(goMod)) {
  console.error('[pre-run] go module is not cpa-account-config-manager:', cwd);
  process.exit(1);
}
...
if (!remoteSet.has('upstream')) {
  const add = git(['remote', 'add', 'upstream', 'https://github.com/Mxucc/cpa-account-config-manager.git']);
  if (add.status !== 0) {
